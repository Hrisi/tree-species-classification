...
def compute_weights(loader, num_class=40): #added
    weights_acc = np.zeros(num_class)

    n_samples, classes = 0, 0
    ...
    weights = np.zeros(num_class)
    for i in range(num_class):
        weights[i] = n_samples / (classes * weights_acc[i])
...
def main():
    ...
    test_loader = DataLoader(ModelNet40(partition='valid', num_points=args.num_points), num_workers=args.workers,
                             batch_size=args.batch_size // 2, shuffle=False, drop_last=False)

    weights = compute_weights(train_loader, num_class=40) #added
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        printf(
                f"Training loss:{train_out['loss']} acc_avg:{train_out['acc_avg']}% acc:{train_out['acc']}% f1: {train_out['f1']}% precision: {train_out['precision']}% recall: {train_out['recall']}% time:{train_out['time']}s")
        printf(
            f"Testing loss:{test_out['loss']} acc_avg:{test_out['acc_avg']}% "
            f"acc:{test_out['acc']}% f1: {test_out['f1']}% precision: {test_out['precision']}% recall: {test_out['recall']}% time:{test_out['time']}s [best test acc: {best_test_acc}%] \n\n")
    logger.close()
...
def train(net, trainloader, optimizer, criterion, device, weights=None):
    net.train()
    train_loss = 0
    correct = 0
    total = 0
    ...
    for batch_idx, (data, label) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        logits = net(data)
        loss = criterion(logits, label, weights=weights)
        loss.backward()
    ...
    return {
        ...
        "f1": float("%.3f" % (100. * metrics.f1_score(train_true, train_pred, average="weighted"))),
        "precision": float("%.3f" % (100. * metrics.precision_score(train_true, train_pred, average="weighted"))),
        "recall": float("%.3f" % (100. * metrics.recall_score(train_true, train_pred, average="weighted"))),
        "time": time_cost
    }
...
def validate(net, testloader, criterion, device):
    ...
    return {
        ...
        "f1": float("%.3f" % (100. * metrics.f1_score(test_true, test_pred, average="weighted"))),
        "precision": float("%.3f" % (100. * metrics.precision_score(test_true, test_pred, average="weighted"))),
        "recall": float("%.3f" % (100. * metrics.recall_score(test_true, test_pred, average="weighted"))),
        "time": time_cost
    }
